...
def dry_run():
    ...
    cur.execute("""
        SELECT i.influencerid, i.username, i.name, i.bio, i.businesscategoryname,
               (SELECT string_agg(caption, ' | ') FROM (SELECT caption FROM posts WHERE ownerid = i.influencerid LIMIT 5) p) as captions
        FROM influencers i
    ...
    for infl_id, username, name, bio, old_cat, captions in samples:
        ...
        with open("niche_dry_run_output.txt", "a") as f:
        ...
        Based on the profile bio and post captions below, classify this influencer into EXACTLY ONE category from the provided list.
        ...
        try:
            ...
            print(f"  Error classifying @{username}: {e}")
        print("-" * 40)
